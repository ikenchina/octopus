...
type ModelStorage interface {
	...
	FindTxnExpired(ctx context.Context, txn string, limit int) ([]*Txn, error)
}
...
// @todo lease
func (ms *modelStorage) FindTxnExpired(ctx context.Context, txnType string, limit int) ([]*Txn, error) {
	ctx, cancel := context.WithTimeout(ctx, ms.timeout)
	defer cancel()
