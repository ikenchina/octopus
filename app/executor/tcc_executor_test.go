...
func (s *_tccSuite) TestNormal() {
	...
	s.checkCommitState(model.TxnStateCommitted, txn1)
...
func (s *_tccSuite) TestCancel() {
	...
	s.Nil(cf.GetError())
	txn1 = cf.Txn
	s.checkRollbackState(model.TxnStateAborted, txn1)
	s.checkStorage(storage, txn1)
	for _, branch := range txn1.Branches {
		h, ok := s.servers.handlers[branch.Action]
		s.True(ok)
...
func (s *_tccSuite) TestCancelRetry() {
	...
	txn1 := s.newTxn()
	branches1 := txn1.Branches
	service1 := s.getServerHandlers(1, model.BranchTypeCancel)
...
func (s *_tccSuite) TestExpiredConfirm() {
	...
	txn1.SetState(model.TxnStateCommitting)
...
func (s *_tccSuite) TestTimeout() {
	...
	txn1 := s.newTxn()
	branches1 := txn1.Branches
	service1 := s.getServerHandlers(1, model.BranchTypeConfirm)
...
func (s *_tccSuite) checkCommitState(state string, sm *model.Txn) {
	...
	for _, bb := range sm.Branches {
		if bb.BranchType == s.commitBranch {
			s.Equal(model.TxnStateCommitted, bb.State)
		}
	}
}
...
func (s *_tccSuite) startExecutor(storage model.ModelStorage) (model.ModelStorage, *TccExecutor, *rmMockHandler, *rmClientMock) {
	s.initServers(model.TxnTypeTcc)
	if storage == nil {
		storage = model.NewModelStorageMock(s.lessee)
	}
	exec, err := NewTccExecutor(storage, 10, s.lessee)
	s.NotNil(exec)
	s.Nil(err)
	s.Nil(exec.Start())
...
func (s *_tccSuite) newTxn() *model.Txn {
	...
	types := []string{model.BranchTypeConfirm, model.BranchTypeCancel}
	for _, branch := range names {
		for _, tt := range types {
			...
			sub := &model.Branch{
				Gtid:        sg.Gtid,
				Bid:         fmt.Sprintf("stid_%d", s.tidCounter),
				BranchType:  tt,
				Action:      hh.url,
