...
func (s *_baseSuite) initServers(txnType string) {
	...
	s.commitBranch = model.BranchTypeConfirm
	s.rollbackBranch = model.BranchTypeCancel

	if txnType == model.TxnTypeSaga {
		s.commitBranch = model.BranchTypeCommit
		s.rollbackBranch = model.BranchTypeCompensation
	}
