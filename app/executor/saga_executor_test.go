...
func (s *_sagaSuite) TestNormal() {
	...
	s.checkState(model.TxnStateCommitted, sagaPm)
...
func (s *_sagaSuite) TestTimeoutRetryOk() {
	...
	// timeout conditions
	service1Handler := s.getServerHandlers(1, model.BranchTypeCommit)
	...
	// check storage model
	s.Equal(model.TxnStatePrepared, sagaPm.State)
	...
	// ############   second time   ############
	s.Equal(model.TxnStateFailed, service1Commit.State)

	// response checking
	s.Nil(sagaFuture.GetError())
	s.checkState(model.TxnStateCommitted, sagaFuture.Txn)
...
func (s *_sagaSuite) TestCompensationOk() {
	...
	s.Nil(err)
	s.Equal(model.TxnStatePrepared, get.State)
	s.Equal(1, service0Commit.TryCount)
	s.Equal(1, service1Commit.TryCount)
	s.Equal(0, service2Commit.TryCount)
	...
	// response checking
	s.Nil(sagaFuture.GetError())
	s.Equal(model.TxnStateAborted, sagaFuture.Txn.State)
	...
	for _, bb := range sagaPm.Branches {
		if bb.BranchType == s.commitBranch || bb.BranchType == model.BranchTypeCompensation {
	...
	s.Equal(model.TxnStateAborted, nr.State)
...
func (s *_sagaSuite) TestCron() {
	...
	s.Equal(model.TxnStatePrepared, sg1.State)
	s.Equal(model.TxnStatePrepared, sg2.State)
	...
	sg1, _ = sagaExec.Get(ctx, sagaFuture.Gtid)
	sg2, _ = sagaExec.Get(ctx, sagaFuture2.Gtid)
	s.Equal(model.TxnStateCommitted, sg1.State)
	s.Equal(model.TxnStateCommitted, sg2.State)
	s.Equal(model.TxnStateCommitted, notifier2.getNotify(sg1.Gtid).State)
	s.Equal(model.TxnStateCommitted, notifier2.getNotify(sg2.Gtid).State)
...
func (s *_sagaSuite) newSage() *model.Txn {
	...
	types := []string{s.commitBranch, model.BranchTypeCompensation}
	for _, branch := range names {
		for _, tt := range types {
			hh := s.servers.getHandler(branch, tt)

			stid := fmt.Sprintf("stid_%d", s.tidCounter)
			s.tidCounter++
			sub := &model.Branch{
				Gtid:        sg.Gtid,
				Bid:         stid,
...
func (s *_sagaSuite) startSagaExecutor(storage model.ModelStorage) (model.ModelStorage, *SagaExecutor, *rmMockHandler, *rmClientMock) {
	...
	s.initServers(model.TxnTypeSaga)
	if storage == nil {
		storage = model.NewModelStorageMock(s.lessee)
	}

	sagaExec, err := NewSagaExecutor(storage, 10, s.lessee)
